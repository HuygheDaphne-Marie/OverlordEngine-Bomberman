...
void SpriteRenderer::UpdateBuffer(const SceneContext& sceneContext)
{
	...
	if (m_pVertexBuffer)
	{
		...
		sceneContext.d3dContext.pDeviceContext->Map(m_pVertexBuffer, 0, D3D11_MAP_WRITE_NO_OVERWRITE, 0, &subRes);

		// use memcpy to copy all our sprite vertices (m_Sprites) to the mapped resource (D3D11_MAPPED_SUBRESOURCE::pData)
		memcpy(subRes.pData, m_Sprites.data(), sizeof(VertexSprite) * m_BufferSize);
