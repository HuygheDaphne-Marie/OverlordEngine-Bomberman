...
#pragma warning(pop)
	...
	//Root-Scene Update
	for (size_t i = 0; i < m_pChildren.size(); i++) // for each was causing crashes
	{
		const auto pChild = m_pChildren[i];
...
#pragma region POST-PROCESSING PASS
	...
	if (m_PostProcessingMaterials.size() > 0)
	{
		...
		RenderTarget* const pInitialRenderTarget{ m_pGame->GetRenderTarget() };
		...
		//		For Each Material
		for (PostProcessingMaterial* const pMaterial : m_PostProcessingMaterials)
		{
			...
			pMaterial->Draw(GetSceneContext(), pPreviousRenderTarget);
		...
		{
			m_pGame->SetRenderTarget(nullptr); // null sets to default

			//		- Use SpriteRenderer::DrawImmediate to render the ShaderResourceView from PREV_RT to the screen
			SpriteRenderer::Get()->DrawImmediate(m_SceneContext.d3dContext, pPreviousRenderTarget->GetColorShaderResourceView(), XMFLOAT2{ 0, 0 });
		}
...
GameObject* GameScene::FindGameObjectWithTag(const std::wstring& tag)
{
	return FindGameObjectWithTag(tag, m_pChildren);
}

GameObject* GameScene::FindGameObjectWithTag(const std::wstring& tag, const std::vector<GameObject*>& collectionToSearch)
{
	for (GameObject* go : collectionToSearch)
	{
		if (go == nullptr)
		{
			continue;
		...
		if (go->GetChildren<GameObject>().size() > 0) // Recursively check children
		{
			return FindGameObjectWithTag(tag, go->GetChildren<GameObject>());
		}
	}
	return nullptr;
}
