...
void CreateVertex(inout TriangleStream<GS_DATA> triStream, float3 pos, float4 col, float2 texCoord, int channel)
{
	...
	geomData.Position = mul(float4(pos, 1.0f), gTransform); // transform pos to screenspace
	geomData.Color = col;
...
void MainGS(point VS_DATA vertex[1], inout TriangleStream<GS_DATA> triStream)
{
	...
	float2 charSize = vertex[0].CharSize; // ease of use
	float2 charTexCoordSize = float2(charSize / gTextureSize);
