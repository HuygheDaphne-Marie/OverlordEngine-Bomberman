...
void Character::Update(const SceneContext& sceneContext)
{
	if (m_pCameraComponent->IsActive())
	{
		...
		if (sceneContext.pInput->IsMouseButton(InputState::down, VK_LBUTTON))
		{
			...
			// No cast availible?
			POINT mouseMovement = sceneContext.pInput->GetMouseMovement();
		...
		float moveAcceleration = m_MoveAcceleration * deltaTime;

		//If the character is moving (= input is pressed)
		if (abs(move.x) > epsilon || abs(move.y) > epsilon)
		{
			//Calculate & Store the current direction (m_CurrentDirection) >> based on the forward/right vectors and the pressed input
			XMStoreFloat3(&m_CurrentDirection, XMVectorAdd(XMVectorScale(forward, move.y), XMVectorScale(right, move.x)));
		...
		XMStoreFloat3(&horizontalVelocity, XMVectorScale(XMLoadFloat3(&m_CurrentDirection), m_MoveSpeed));
		...
		XMStoreFloat3(&displacement, XMVectorScale(XMLoadFloat3(&m_TotalVelocity), deltaTime));
